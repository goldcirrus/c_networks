...
/* Create a TCP client stream socket: OS knows its socket address fields value, so no need to explictly define its socket address. */
	...
	/*The following code won't execute unless server kills the connection first to end the above while loop....If the server doesn't kill the connection[server end: control+c], then the above while loop will runs forever. If the client end kills the connection first by [control+c], the following codes won't execute neither. byebye and program terminates.   */

	printf("\nPrint outside of while loop which keeping recv(): Only happen after server kills the connect first----The echoed back message again is  %s.\n", buffer);
        
    
	// Close the only client socket
	close (socket_fd);
