...
int swap_write(int sd, unsigned char *buf, int length)
{
        ...
        frame[0] = s+'0';              //frame# is added as the 1st element of frame array
 ...
 //             printf("\t swap_write send frame:%s \n", frame);
...
//                       printf("\nACK and Checksum received correctly from server. To send another frame\n");
                         return numSent;         //return the length sent, and stop loop, terminate this function. 
                           //swap_write() will be called again and get passed in new message to sent out
                    }               
                    if(checkAck==ack[4] && ack[0]==(s-1+'0'))     //if receive duplicated ACK
                    {     
//                       //received the last ACK again: do nothing, alread send frame according to last ACK
                         return 1;           //retrun 1; terminate this funtion, start new write again
                    }
