...
/* Forever in while loop to listen and accept connections request from client...*/
        while(1)    
        {
              ...
              if((s_socket = accept(listen_socket,(struct sockaddr*)&client_address,(socklen_t*)&client_address_len))==-1)
               ...
               printf("Tell: listen_socket accepts a client request return a socket for data transmit. ");
              ...
              /*keep receiving as long as the current client keeps the connection */
               ...
               /*the code below won't be executed since as long as the client is connected to the server, it is trapped in the while loop forever. When the connection is forced break. the send() function lost the s_socket to send to.  */
               printf("butffer content to be sent out is %s:\n", buffer);
               /*after receiving finishes, send back(echo) all bytes(characters) received in buffer to client*/
               send(s_socket,buffer,len,0);
...
//close(listen_socket) here will cause server terminate listening when the first client finish its turn
               close(s_socket);    //close data transmit socket for the current cient finishes.      
         }     	


		
         close(listen_socket);				// Cloese descriptor referencing server socket
	 return 0;
}                                              // End main
